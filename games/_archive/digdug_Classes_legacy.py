...
class Player(pygame.sprite.Sprite):         #Create Player
    def __init__(self):                     #Set Attributes and Functions
        ...
        try:
            self.imageR = pygame.image.load(weasel_path).convert_alpha()
            self.imageR = pygame.transform.scale(self.imageR, (100, 100))
        except:
...
class Enemy1(pygame.sprite.Sprite):#Create Enemy
    def __init__(self):
        super().__init__()
        # Load soupcan_new.png as enemy (with translucency)
        # Preserve aspect ratio to avoid distortion
        soupcan_path = os.path.join(_media_dir, 'soupcan_new.png')
        try:
            self.imagel = pygame.image.load(soupcan_path).convert_alpha()
            # Scale preserving aspect ratio
            orig_w, orig_h = self.imagel.get_size()
            target_size = 100
            scale = min(target_size / orig_w, target_size / orig_h)
            new_w, new_h = int(orig_w * scale), int(orig_h * scale)
            self.imagel = pygame.transform.scale(self.imagel, (new_w, new_h))
            # Apply translucency to match other game sprites
            self.imagel.set_alpha(200)
        except:
...
class Objective(pygame.sprite.Sprite):#Create Objective
    def __init__(self):
        super().__init__()
        # Load soupbowl.png as collectible
        soupbowl_path = os.path.join(_media_dir, 'soupbowl.png')
        try:
            self.image = pygame.image.load(soupbowl_path).convert_alpha()
